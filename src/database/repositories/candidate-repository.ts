...
export interface FilterCandidateRow {
  ...
  price_cents: number; review_count: number | null; rating: number | null; enriched_image_url: string; child_sales_30d: number | null;
  ...
  prefilter(): { occurrenceCount: number; eligibleOccurrences: number; uniqueCandidates: number } {
    ...
    return this.db.transaction(() => {
      ...
      this.db.prepare(`WITH eligible AS (
        ...
        WHERE (o.rating IS NULL OR o.rating>=?) AND (o.review_count IS NULL OR o.review_count<?) AND o.price_cents IS NOT NULL AND o.price_cents>=? AND o.price_cents<=?
